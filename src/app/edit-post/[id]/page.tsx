...
function TextOverlayEditor({
  ...
  const visibleElements = elements.filter(el => {
    ...
    if (el.photoIndex === undefined) return true;
...
export default function EditPost() {
  ...
  if (showTextEditor) {
    const isPhotoCarousel = postType === "photo" && mediaUrls.length > 1;
    const currentPreview = isPhotoCarousel ? (mediaUrls[carouselIndex] || mediaUrl) : (mediaUrl || null);
    return (
      <TextOverlayEditor
        ...
        {mediaUrl && (
          <div style={{ marginBottom: 20, borderRadius: 16, overflow: "hidden", height: 220, position: "relative", background: "#000" }}>
            {postType === "video"
              ...
              : <img src={mediaUrls.length > 1 ? mediaUrls[carouselIndex] : mediaUrl} style={{ width: "100%", height: "100%", objectFit: "contain" }} />}
            {textElements
              .filter(el => {
                if (postType !== "photo" || mediaUrls.length <= 1) return true;
                if (el.photoIndex === undefined) return true;
              ...
              <>
                {carouselIndex > 0 && (
                  <button onClick={() => setCarouselIndex(c => c - 1)}
                    style={{ position: "absolute", left: 8, top: "50%", transform: "translateY(-50%)", width: 30, height: 30, borderRadius: "50%", background: "rgba(0,0,0,.55)", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
                    <svg width="12" height="12" viewBox="0 0 14 14" fill="none" stroke="#fff" strokeWidth="2"><path d="M9 2L4 7l5 5" strokeLinecap="round" strokeLinejoin="round" /></svg>
                  </button>
                )}
                {carouselIndex < mediaUrls.length - 1 && (
                  <button onClick={() => setCarouselIndex(c => c + 1)}
                    style={{ position: "absolute", right: 50, top: "50%", transform: "translateY(-50%)", width: 30, height: 30, borderRadius: "50%", background: "rgba(0,0,0,.55)", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
                    <svg width="12" height="12" viewBox="0 0 14 14" fill="none" stroke="#fff" strokeWidth="2"><path d="M5 2l5 5-5 5" strokeLinecap="round" strokeLinejoin="round" /></svg>
                  </button>
                )}
