...
export default function Feed() {
  ...
  function goNext() {
    setCurrent(c => { const n = Math.min(postsRef.current.length - 1, c + 1); currentRef.current = n; return n; });
    setCarouselIndex(0);
  }
  function goPrev() {
    setCurrent(c => { const n = Math.max(0, c - 1); currentRef.current = n; return n; });
    setCarouselIndex(0);
  }
    ...
    async function loadPosts() {
      try {
        ...
        if (user) {
          ...
          setFollowingIds(ids);
          const { data } = await supabase.from("posts").select("*, profiles(username, full_name, avatar_url)").order("created_at", { ascending: false }).limit(50);
  ...
  useEffect(() => {
    ...
    async function loadFollowedPosts() {
      ...
      try {
        ...
        const { data } = await supabase.from("posts").select("*, profiles(username, full_name, avatar_url)").in("user_id", followingIds).in("visibility", ["public", "friends"]).order("created_at", { ascending: false }).limit(20);
  ...
  useEffect(() => {
    ...
    if (post?.musicUrl) {
      ...
      audio.volume = volume; audio.loop = true;
  ...
  function handleUnmute() {
    ...
    if (musicRef.current) { if (newMuted) musicRef.current.pause(); else musicRef.current.play().catch(() => {}); }
      ...
      <div className="zz-info" style={{ position: "absolute", bottom: 100, left: 16, right: 80, zIndex: 20 }}>
        <div style={{ display: "flex", flexDirection: "column", gap: 8, maxWidth: 480 }}>
          ...
          {post.hasLink && post.linkName && (
            <a href={post.linkName.startsWith("http") ? post.linkName : `https://${post.linkName}`} target="_blank" rel="noopener noreferrer"
      ...
      <div style={{ position: "absolute", right: 12, bottom: 100, zIndex: 30, display: "flex", flexDirection: "column", alignItems: "center", gap: 12 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
          <button onClick={handleUnmute} style={{ width: 48, height: 48, borderRadius: "50%", background: muted ? "rgba(255,77,77,.2)" : "rgba(255,255,255,.15)", border: muted ? "1.5px solid rgba(255,77,77,.5)" : "none", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
            {muted ? (
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#FF4D4D" strokeWidth="2"><path d="M11 5L6 9H2v6h4l5 4V5z" /><line x1="23" y1="9" x2="17" y2="15" strokeLinecap="round" /><line x1="17" y1="9" x2="23" y2="15" strokeLinecap="round" /></svg>
            ) : (
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2"><path d="M11 5L6 9H2v6h4l5 4V5z" /><path d="M15.54 8.46a5 5 0 0 1 0 7.07" strokeLinecap="round" /><path d="M19.07 4.93a10 10 0 0 1 0 14.14" strokeLinecap="round" /></svg>
            )}
          </button>
          ...
          <div style={{ width: "100%", borderRadius: "20px 20px 0 0", background: "#111", border: "0.5px solid rgba(255,255,255,.1)", padding: "20px 20px 40px" }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
              <button onClick={() => { if (navigator.share) navigator.share({ title: post.user, text: post.caption, url: window.location.href }); else { navigator.clipboard.writeText(window.location.href); setShareMsg("Link copiato!"); } setShowShare(false); }}
                ...
                <div style={{ textAlign: "left" }}><div style={{ color: "#fff", fontWeight: 600, fontSize: 14 }}>Condividi</div><div style={{ color: "rgba(255,255,255,.4)", fontSize: 11, marginTop: 2 }}>Invia tramite app</div></div>
              </button>
              <button onClick={() => { navigator.clipboard.writeText(window.location.href); setShareMsg("Link copiato! ✓"); setTimeout(() => setShareMsg(""), 2000); }}
                ...
                <div style={{ textAlign: "left" }}><div style={{ color: "#fff", fontWeight: 600, fontSize: 14 }}>Copia link</div><div style={{ color: "rgba(255,255,255,.4)", fontSize: 11, marginTop: 2 }}>Copia il link del post</div></div>
              </button>
            </div>
            {shareMsg && <div style={{ marginTop: 16, padding: "10px 16px", borderRadius: 12, background: "rgba(29,158,117,.15)", border: "1px solid rgba(29,158,117,.3)", color: "#4dffb8", fontSize: 13, fontWeight: 600, textAlign: "center" }}>{shareMsg}</div>}
          </div>
        </div>
      )}
